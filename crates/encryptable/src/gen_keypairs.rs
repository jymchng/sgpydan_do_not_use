...
use serde_encrypt::{
    ...
    AsSharedKey, ReceiverKeyPairCore, SenderCombinedKey, SenderKeyPairCore,
};

static B64_ENCODE: GeneralPurpose = general_purpose::STANDARD_NO_PAD;
...
fn main() {
    ...
    match matches.subcommand() {
        Some(("sender", _)) => {
            ...
            println!(
                "Public key: {:#?}",
                B64_ENCODE.encode(sender_key_pair.public_key().as_ref().as_bytes()),
            );
            println!(
                "Private key: {:#?}",
                B64_ENCODE.encode(sender_key_pair.private_key().as_ref().to_bytes()),
            );
        }
        Some(("receiver", _)) => {
            ...
            println!(
                "Public key: {:#?}",
                B64_ENCODE.encode(receiver_key_pair.public_key().as_ref().as_bytes())
            );
            println!(
                "Private key: {:#?}",
                B64_ENCODE.encode(receiver_key_pair.private_key().as_ref().to_bytes())
            );
        }
